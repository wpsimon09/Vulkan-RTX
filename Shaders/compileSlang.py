...
def compile_shader(input_path,command, success_msg, fail_msg, verbose):
    """Compile a single shader file."""
    if not has_changed_with_git(input_path):
        if verbose:
            print(f"✓ Skipped (unchanged): {input_path}")
        return False
    try:
...
def compile_bundled_shaders(folder, verbose):
...
def compile_raytracing_shaders(folder, verbose):
...
def main():
    ...
    if not os.path.isfile(SLANGC_PATH) or not os.access(SLANGC_PATH, os.X_OK):
        print(f"Error: slangc not found or not executable at {SLANGC_PATH}")
        return
    ...
    if args.stage in ("env", "all"):
        ...
        compile_bundled_shaders("Source/EnvGeneration", args.verbose)

    if args.stage in ("depth", "all"):
        print("== Depth Pre-Pass ==")
        compile_bundled_shaders("Source/DepthPrePass", args.verbose)

    if args.stage in ("postprocessing", "all"):
        print("== Post-Processing ==")
        compile_bundled_shaders("Source/PostProcessing", args.verbose)

    if args.stage in ("raytracing", "all"):
        print("== Ray Tracing ==")
        compile_raytracing_shaders("Source/RTX", args.verbose)

    if args.stage in ("rtshadows", "all"):
        print("== RT Shadows ==")
        compile_bundled_shaders("Source/RTShadowPass", args.verbose)
