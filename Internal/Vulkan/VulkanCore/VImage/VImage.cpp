...
VulkanCore::VImage::VImage(const VulkanCore::VDevice &device, std::optional<vk::Image> image, int widht, int height,
                           uint32_t mipLevels, vk::Format format, vk::ImageAspectFlags aspecFlags):
    m_device(device), m_imageVK(image.value_or(VK_NULL_HANDLE)), m_mipLevels(mipLevels), m_format(format), m_aspectFlags(aspecFlags), m_width(widht), m_height(height) {
