...
VulkanCore::VImage::VImage(const VulkanCore::VDevice &device, vk::Image image, int widht, int height,
    ...
    m_device(device), m_imageVK(image), m_mipLevels(mipLevels), m_format(format), m_aspectFlags(aspecFlags) {
...
VulkanCore::VImage::VImage(const VulkanCore::VDevice &device, uint32_t mipLevels,
    ...
    if(aspecFlags & vk::ImageAspectFlagBits::eColor) {
        ...
        GlobalVariables::GlobalStructs::ImageData dummyImageData = {};
        ...
        dummyImageData.pixels = {0xFFFF69B4};
...
void VulkanCore::VImage::FillWithImageData(GlobalVariables::GlobalStructs::ImageData &imageData) {
...
void VulkanCore::VImage::CopyFromBufferToImage() {
    ...
    assert(
        m_transferCommandBuffer->GetIsRecording() && "Command buffer that will copy buffer to image is not recording");
