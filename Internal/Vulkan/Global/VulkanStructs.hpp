...
struct DrawCallData
{
  ...
  friend bool operator==(const DrawCallData& lhs, const ObjectDataUniform& rhs)
  {
    if(auto* lhsPBRMat = dynamic_cast<ApplicationCore::PBRMaterial*>(lhs.material))
    {
      ...
      return lhsMatDescription.features == rhsMatDescription.features && lhsMatDescription.values == rhsMatDescription.values
             && lhs.modelMatrix == rhs.model && lhs.position == rhs.position;
    }
    else
    {
