...
namespace Renderer {
FogPass::FogPass(const VulkanCore::VDevice& device, ApplicationCore::EffectsLibrary& effectLibrary, int width, int height): RenderPass(device, width, height),m_parameters{}
{
   ...
   m_renderTargets[0] = std::make_unique<Renderer::RenderTarget2>(m_device, fogPassOutputCI);
...
void FogPass::Init(int currentFrameIndex, VulkanUtils::VUniformBufferManager& uniformBufferManager, VulkanUtils::RenderContext* renderContext)
{
  ...
  e->WriteImage(currentFrameIndex, 0, 1,
                renderContext->visibilityBuffer->GetDescriptorImageInfo(
                    VulkanCore::VSamplers::Sampler2D));
  e->WriteImage(currentFrameIndex, 0, 2,
                renderContext->positionMap->GetDescriptorImageInfo(
  ...
  e->WriteImage(currentFrameIndex, 0, 3,
                MathUtils::LookUpTables.BlueNoise1024->GetHandle()->GetDescriptorImageInfo(
                    VulkanCore::VSamplers::Sampler2D));
  e->WriteImage(currentFrameIndex, 0, 4,
                renderContext->lightPassOutput->GetDescriptorImageInfo(
                    VulkanCore::VSamplers::Sampler2D));
...
void FogPass::Render(int currentFrame, VulkanCore::VCommandBuffer& cmdBuffer, VulkanUtils::RenderContext* renderContext)
{
  ...
  m_renderTargets[0]->TransitionAttachments(cmdBuffer, vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
