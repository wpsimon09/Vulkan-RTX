...
RenderTarget::RenderTarget(const VulkanCore::VDevice& device, const VulkanCore::VSwapChain& swapChain)
    : m_device(device)
...
void RenderTarget::CreateRenderTargetForSwapChain(const VulkanCore::VSwapChain& swapChain)
{
  ...
  {
    m_colourAttachments[i].second =
        std::make_unique<VulkanCore::VImage2>(m_device, colourAttachmentCreateInfo, swapChainImages[i]);
