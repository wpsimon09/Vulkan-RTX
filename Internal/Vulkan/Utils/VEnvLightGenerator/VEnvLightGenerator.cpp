                                                   ...
                                                   VulkanCore::VTimelineSemaphore&      renderingSemaphore)
{

  {
    ...
    //=============================================================
    {
      ...
      std::vector<std::unique_ptr<VUniform<PushBlock>>> hdrPushBlocks(6 * mipLevels);
      ...
      for(int mip = 0; mip < mipLevels; mip++)
      {
        ...
        for(int face = 0; face < 6; face++)
        {
          ...
          cmdBuffer.pushDescriptorSetWithTemplateKHR(hdrToCubeMapEffect.GetUpdateTemplate(),
                                                     hdrToCubeMapEffect.GetPipelineLayout(), 0, updateStuct, m_device.DispatchLoader);
          ...
          CopyResukt(cmdBuffer, renderTarget->GetImage(), hdrCubeMap->GetImage(), viewport.width, viewport.height, mip, face);
...
{
  {
    ...
    //=============================================================
    {
      ...
      for(int face = 0; face < 6; face++)
      {
        ...
        cmdBuffer.pushDescriptorSetWithTemplateKHR(cubeMapToIrradianceEffect.GetUpdateTemplate(),
                                                   cubeMapToIrradianceEffect.GetPipelineLayout(), 0, updateStuct,
        ...
        //=================== cpy offscreen immage to the cueb map`s face
        CopyResukt(cmdBuffer, renderTarget->GetImage(), irradianceCubeMap->GetImage(), viewport.width, viewport.height, 0, face);
  ...
  //=============================================================
  {
    ...
    for(int mipLevel = 0; mipLevel < mipLevels; mipLevel++)
    {
      for(int face = 0; face < 6; face++)
      {
        ...
        cmdBuffer.pushDescriptorSetWithTemplateKHR(hdrToPrefilterEffect.GetUpdateTemplate(),
                                                   hdrToPrefilterEffect.GetPipelineLayout(), 0, updateStuct, m_device.DispatchLoader);
        ...
        CopyResukt(cmdBuffer, renderTarget->GetImage(), prefilterMap->GetImage(), viewport.width, viewport.height, mipLevel, face);
...
void VulkanUtils::VEnvLightGenerator::GenerateBRDFLut()
{
  ...
  RecordImageTransitionLayoutCommand(*m_brdfLut, vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::eUndefined,
                                     *m_transferCmdBuffer);
