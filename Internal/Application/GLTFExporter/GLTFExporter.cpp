...
ApplicationCore::GLTFExporter::GLTFExporter()
    : m_lightInfo(GlobalVariables::lightInfoPath)
    , m_lightInfoPath(GlobalVariables::configFolder / "Light.ini")
{
}

void ApplicationCore::GLTFExporter::ExportScene(std::filesystem::path path, Scene& scene, AssetsManager& assetsManager)
{
...
void ApplicationCore::GLTFExporter::ParseBuffers(fastgltf::Asset& asset, AssetsManager& assetsManager)
{
  ...
  std::vector<std::byte> vertexBufferVector(reinterpret_cast<std::byte*>(vertices.data()),
                                            reinterpret_cast<std::byte*>(vertices.data()) + m_vertexBuffer.byteLength);
  ...
  std::vector<std::byte> indexBufferVector(reinterpret_cast<std::byte*>(indices.data()),
                                           reinterpret_cast<std::byte*>(indices.data()) + m_indexBuffer.byteLength);
...
void ApplicationCore::GLTFExporter::ParseScene(std::shared_ptr<SceneNode> sceneNode, AssetsManager& assetsManager, fastgltf::Asset& asset)
{
  ...
  if(IsNodeValid(sceneNode))
  {
    ...
    trs.translation = fastgltf::math::vec<float, 3>(sceneNode->m_transformation->GetPosition().x,
                                                    sceneNode->m_transformation->GetPosition().y,
                                                    sceneNode->m_transformation->GetPosition().z);
    trs.rotation    = fastgltf_Quat;
    trs.scale       = fastgltf::math::vec<float, 3>(sceneNode->m_transformation->GetScale().x,
                                                    sceneNode->m_transformation->GetScale().y,
                                                    sceneNode->m_transformation->GetScale().z);
...
void ApplicationCore::GLTFExporter::ParseMaterial(fastgltf::Asset& asset, AssetsManager& assetsManager)
{
  ...
  for(auto& mat : assetsManager.GetAllMaterials())
  {
    ...
    material.pbrData.baseColorFactor =
        fastgltf::math::vec<float, 4>(matValues.diffuse.x, matValues.diffuse.y, matValues.diffuse.z, matValues.diffuse.w);
...
void ApplicationCore::GLTFExporter::ParseLights(Scene& scene)
{
  ...
  for(int i = 0; i < lightInfo.PointLightInfos.size(); i++)
  {
    ...
    if(pointLight != nullptr)
    {
      ...
      {
        ini[pointLightKey].set({{"colR", std::to_string(pointLight->colour.x)},
  ...
  for(int i = 0; i < lightInfo.AreaLightInfos.size(); i++)
  {
    ...
    {
      ini[areaLightKey].set({{"colR", std::to_string(areaLight->colour.x)},
