...
ApplicationCore::VTextureAsset::VTextureAsset(const VulkanCore::VDevice&           device,
                                              std::shared_ptr<VulkanCore::VImage2> defaultTexture,
                                              ETextureAssetType                    type,
                                              std::filesystem::path                texturePath)
    : VAsset<VulkanCore::VImage2>(device)
    , m_textureAssetType(type)
    , m_originalPathToTexture(texturePath)
...
ApplicationCore::VTextureAsset::VTextureAsset(const VulkanCore::VDevice&           device,
    ...
    : VAsset<VulkanCore::VImage2>(device)
    , m_textureAssetType(type)
    , m_textureBufferInfo(bufferInfo)
    , m_transferOpsManager(device.GetTransferOpsManager())
{
    m_deviceHandle  = defaultTexture;
    m_width         = 1;
...
ApplicationCore::VTextureAsset::VTextureAsset(const VulkanCore::VDevice& device, std::shared_ptr<VulkanCore::VImage2> texture)
    : VAsset<VulkanCore::VImage2>(device)
    , m_textureAssetType(ETextureAssetType::Texture)
    , m_textureBufferInfo(nullptr)
    , m_transferOpsManager(device.GetTransferOpsManager())
{
