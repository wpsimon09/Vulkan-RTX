...
std::vector<std::shared_ptr<SceneNode>> GLTFLoader::LoadGLTFScene(std::filesystem::path gltfPath, const ImportOptions& importOptions) const
{
  ...
  else
  {
    ...
    else
    {
      ...
      MaterialPaths                paths = {.saveToDisk = true};
      std::shared_ptr<PBRMaterial> material =
          std::make_shared<ApplicationCore::PBRMaterial>(m_assetsManager.GetEffects()[EEffectType::ForwardShader], paths, m_assetsManager);
      materials.emplace_back(material);
      m_assetsManager.m_materials.emplace_back(material);
    }
    ...
    fastgltf::iterateSceneNodes(gltf, 0, fastgltf::math::fmat4x4(), [&](fastgltf::Node& node, fastgltf::math::fmat4x4 matrix) {
      ...
      Transformations transformations(glm::vec3(transform->translation.x(), transform->translation.y(),
