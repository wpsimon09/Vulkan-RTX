...
void ApplicationCore::LoadSceneLights(ApplicationCore::Scene& scene, std::filesystem::path& lightInfoPath)
{
    ...
    for (int i = 0; i < numOfAreaLights; i++)
    {
        ...
        areaLightData.colour = {
            std::stof(ini[areaLightKey]["colr"]),
            std::stof(ini[areaLightKey]["colg"]),
        ...
        areaLightData.scale = {
            std::stof(ini[areaLightKey]["scalex"]),
            std::stof(ini[areaLightKey]["scaley"])
        };

        areaLightData.orientation = {
            std::stof(ini[areaLightKey]["quatX"]),
            std::stof(ini[areaLightKey]["quatY"]),
            std::stof(ini[areaLightKey]["quatZ"]),
            std::stof(ini[areaLightKey]["quatW"])
        };
