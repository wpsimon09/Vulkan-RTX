...
void DetailsPanel::Render()
{
    ...
    else
    {
        ...
        if(m_selectedSceneNode->GetSceneNodeMetaData().nodeType == ENodeType::Atmosphere)
        {
            RenderAtmosphereDetails();
        }
    }
...
void DetailsPanel::RenderAtmosphereDetails()
{
    if(auto atmosNode = dynamic_cast<ApplicationCore::AtmosphereSceneNode*>(m_selectedSceneNode.get()))
    {
        ...
        const auto& defaultParams = atmosNode->GetDefaultAtmosphereParams();
