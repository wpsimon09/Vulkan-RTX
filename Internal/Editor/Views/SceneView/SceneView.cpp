...
namespace VEditor {
    ...
    {
        if (ImGui::TreeNodeEx(ICON_FA_LIGHTBULB" Lights", ImGuiTreeNodeFlags_OpenOnArrow
                              | ImGuiTreeNodeFlags_OpenOnDoubleClick))
        {
            for (auto& sceneNode : sceneLights)
            {
                ...
                if (ImGui::TreeNodeEx(GenerateNodeLabel(sceneNode).c_str(), ImGuiTreeNodeFlags_Leaf | (isSelected ? ImGuiTreeNodeFlags_Selected : 0)))
                {
